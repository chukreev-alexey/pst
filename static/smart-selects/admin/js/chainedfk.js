      ...
      init: function (chainfield, url, id, init_value, empty_label, auto_choose) {
        ...
        $(chainfield).change(function (event) {
          ...
          if (localID.indexOf("__prefix__") > -1) {
            var idNumbers = $(this).attr("id").match(/\d+/);
            if (!idNumbers) {
              return event;
            }
            prefix = idNumbers[0];
          ...
          var VALUE_FIELD = document.querySelector('*[data-chainfield="' + this.name + '"]');
          if (VALUE_FIELD) {
            var WORKED_ID = '#' + VALUE_FIELD.id;

            start_value = $(WORKED_ID).val();
            this_val = $(this).val();

            fill_field(this_val, start_value, WORKED_ID, url, empty_label, auto_choose);
